...
struct LoopPlugin : public llvm::PassInfoMixin<LoopPlugin> {
  llvm::PreservedAnalyses run(llvm::Function &Func,
    llvm::FunctionAnalysisManager &Func_analys_manag) {
    ...
    llvm::LoopAnalysis::Result &LI = Func_analys_manag.getResult<llvm::LoopAnalysis>(Func);
    for (auto *Loop : LI) {
  ...
  bool LoopCallPresent(const std::string &LoopFuncName,
  ...
  bool
    LastBlock(llvm::BasicBlock *const BasicB,
